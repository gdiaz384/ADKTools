...
::echo %filepath%  %filename%  %extension%

::if /i "%filepath%" equ "nul" (set parsedPath=%filename%) else (set parsedPath=%filepath%\%filename%)
echo pie1
...
:toWimFromESD
dism /export-image /sourceimagefile:"%parsedPath%.%extension%" /sourceindex:1 /destinationimagefile:"%parsedPath%.wim" /compress:fast /checkintegrity
goto end

::convertwim towim d:\images\install.swm
:toWimFromSWM
dism /export-image /sourceimagefile:"%parsedPath%.%extension%" /swmfile:"%parsedPath%*.%extension%" /sourceindex:1 /destinationimagefile:"%parsedPath%.wim" /compress:fast /checkintegrity
goto end
...
dism /Capture-Image /WIMBoot /ImageFile:%filepath%\%filename%_wimBoot.%extension% /CaptureDir:%default_mountPath% /Name:"Windows %version1% WIMBoot" /ScratchDir:%temp%\scratchDir
